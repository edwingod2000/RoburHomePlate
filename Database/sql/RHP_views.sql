...
-- ------------------------------------------------------
  ...
  union
   select case svr.code  when 'LAATSTE_UPDATE_WED' then  'Wedstrijden'
           when 'LAATSTE_UPDATE_DOC' then  'Documenten'
           when 'LAATSTE_UPDATE_GAS' then  'Gastenboek' end as omschrijving
        , case svr.code  when 'LAATSTE_UPDATE_WED' then  'Games'
           when 'LAATSTE_UPDATE_DOC' then  'Documents'
           when 'LAATSTE_UPDATE_GAS' then  'Guestbook' end as omschrijving
        , str_to_date(svr.waarde, 'DD-MM-YYYY') as datum
     from bsm_sysvar svr
...
-- ------------------------------------------------------
  ...
  AND wed.datum         < DATE(sysdate() + 7)
  AND wed.datum        >= DATE(sysdate() - 1)
  AND wed.archief       = 'N'
  ...
  WHERE wed1.datum < DATE(sysdate() + 7)
  AND wed1.datum  >= DATE(sysdate() - 1)
  ORDER BY datum ,
    soort ,
    tijdstip;
